...
// columns drop out of the discovery mid-run. So the Settings controls can read it
// directly and the "Signals used" dropdown keeps its options during a run. `...Live` is the
// same detection without retention — the fallback for the case retention can't cover, a block
// configured while an upstream block runs, which has no earlier stable value to hold.
const availability = computed(
  () => app.model.outputs.featureAvailability ?? app.model.outputs.featureAvailabilityLive,
);
